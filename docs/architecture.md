...
static product shell     local server + AI/tracker host shell
```
...
- RoleFit server routes delegate to focused modules for AI, job import,
  workspace, applications, and extension behavior. `server.ts` is composition
  and routing, not a catch-all service.
