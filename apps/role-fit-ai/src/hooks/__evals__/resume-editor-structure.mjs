...
import { isFieldFullyMarked, setFieldMark } from "../../lib/inlineMarksText.ts";
...
const seeded = rootReducer(
  { data: null, dirty: false, manualEdited: false, past: [], future: [], coalesceKey: null, coalesceAt: 0 },
...
assert.equal(isFieldFullyMarked("<b>Foo</b>", "bold"), true, "a fully bold-wrapped field reads as marked");
assert.equal(isFieldFullyMarked("<b>A</b> B", "bold"), false, "a partially bold field is not fully marked");
assert.equal(isFieldFullyMarked("Foo", "bold"), false, "a plain field is not marked");
...
const boldedTitles = reduceResumeData(emphasisBase, { type: "setEntriesMark", field: "title", mark: "bold", on: true });
...
const unbolded = reduceResumeData(boldedTitles, { type: "setEntriesMark", field: "title", mark: "bold", on: false });
