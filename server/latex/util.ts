...
// the client scorer/parser (src/resume/sections.ts) — re-exported so the templates
// and the text parser keep importing it from ./util.mjs.
import { isSummaryHeading } from "../../src/resume/sections.ts";
...
export function isSummarySection(section: { type?: "skills" | "summary" | "standard"; heading: string }): boolean {
...
function inlineMarkupToTex(value: unknown): string {
...
export function escapeTex(value: unknown): string {
  let out = String(value ?? "");
...
const LATEX_URL_ESCAPES: [RegExp, string][] = [
  [/\\/g, "\\textbackslash "],
  [/\{/g, "\\{"],
  [/\}/g, "\\}"],
  [/%/g, "\\%"],
...
export function escapeTexUrl(value: unknown): string {
...
function ensureWwwHref(url: string): string {
...
export function linkify(contactItem: unknown): { url: string; label: string } | null {
...
export function splitLinkSegments(text: unknown): { link?: { url: string; label: string }; text?: string }[] {
