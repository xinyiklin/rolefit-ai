...
// mode or commands are mapped; everything else is plain-escaped. A blank divider
// collapses to a thin space so contact items still separate.
...
function styleToLatex(docStyle) {
  ...
  return {
    ...
    // The contact separator is a fixed-width box (from contactGap) holding the
    // user's chosen divider glyph — mirrors the editor's --doc-contact-gap.
    contactSeparatorTex: `\\makebox[${fmtPt(contactGapEm * 10)}][c]{${latexDivider(docStyle.contactDivider)}}`,
