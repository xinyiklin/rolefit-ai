...
import { escapeTex, escapeTexUrl, titleCase, linkify, splitLinkSegments, isSummarySection } from "../util.ts";

// The canonical resume schema this template renders. Both producers — the text
// parser (parseResumeText) and the editor-schema normalizer (normalizeSchema in
// ../index.ts) — emit this all-string shape, so every leaf is a plain string.
type ResumeItem = {
  title: string;
  subtitle: string;
  meta: string;
...
// The client-supplied DocStyle is boundary data: any JSON object (or nothing).
// styleToLatex reads each field defensively via num()/bool()/typeof checks, so the
// fields stay `unknown` and are coerced, never validated.
type DocStyleInput = Record<string, unknown> | null | undefined;

// The resolved LaTeX spacing/typography style buildPreamble + the renderers read.
// entryGap/headerSection gaps are nullable so the no-docStyle fallback can omit them.
type LatexStyle = {
  usesDocStyle: boolean;
  nameContactVSpacePt: number;
  contactSeparatorTex: string;
  headerSectionVSpacePt: number | null;
  sectionGapPt: number;
  sectionEntryGapPt: number;
  entryGapPt: number | null;
  titleSubVSpacePt: number;
  headBulletPt: number;
...
function styleToLatex(docStyle: DocStyleInput): LatexStyle {
...
function bf(arg: string, on: boolean): string {
  return on ? `\\textbf{${arg}}` : arg;
}
function it(arg: string, on: boolean): string {
...
function dividerToTex(divider: unknown): string {
...
function buildPreamble(style: LatexStyle): string {
  const entryGapMacro =
...
function renderHeader(resume: ResumeSchema, style: LatexStyle): string {
...
function renderMaybeLink(text: string): string {
...
// stays plain.
function renderSkillBullet(bullet: string, boldLabel: boolean): string {
...
function renderSkillRowSep(style: LatexStyle): string {
  return style.skillsRowGapPt ? `\\par\\vspace{${fmtPt(style.skillsRowGapPt)}}\n        ` : "\\par\n        ";
}

function renderItem(item: ResumeItem, style: LatexStyle): string {
...
function renderSection(section: ResumeSection, style: LatexStyle): string {
...
export default {
  ...
  render(resume: ResumeSchema, docStyle: DocStyleInput): string {
