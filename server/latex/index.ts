// Public entry for the LaTeX subsystem. server.ts imports from here.
...
import { parseResumeText, extractPlainTextFromLatex } from "./parseResumeText.ts";
import { listTemplates, getTemplate, defaultTemplateId } from "./templates/index.ts";

export { parseResumeText, extractPlainTextFromLatex, listTemplates, defaultTemplateId };

// The canonical resume schema the templates render (matches jakes.ts ResumeSchema).
type LatexItem = {
  title: string;
  subtitle: string;
  meta: string;
...
type LatexSection = { heading: string; type?: "skills" | "summary" | "standard"; items: LatexItem[] };
type LatexSchema = { name: string; contact: string[]; sections: LatexSection[] };
// Client-supplied DocStyle is boundary JSON; the template coerces every field.
type DocStyleInput = Record<string, unknown> | null | undefined;
...
// malformed input.
export function renderResumeTexFromSchema({
  schema,
  templateId,
  docStyle
}: {
...
function normalizeSchema(schema: unknown): LatexSchema {
  const source = (schema && typeof schema === "object" ? schema : {}) as Record<string, unknown>;
  return {
    name: asString(source.name),
    contact: Array.isArray(source.contact) ? source.contact.filter((c) => typeof c === "string") : [],
    sections: Array.isArray(source.sections)
      ? (source.sections.map(normalizeSection).filter(Boolean) as LatexSection[])
      : []
  };
}

function normalizeSection(section: Record<string, unknown> | null | undefined): LatexSection | null {
  ...
  return {
    ...
    items: Array.isArray(section.items)
      ? (section.items.map(type === "skills" ? normalizeSkillItem : normalizeItem).filter(Boolean) as LatexItem[])
      : []
  };
}

function normalizeSkillItem(item: Record<string, unknown> | null | undefined): LatexItem | null {
...
function normalizeItem(item: Record<string, unknown> | null | undefined): LatexItem | null {
  if (!item || typeof item !== "object") return null;
  return {
...
let tectonicCheck: Promise<{ available: boolean; version: string | null }> | null = null;

export function checkTectonicAvailability(): Promise<{ available: boolean; version: string | null }> {
  if (tectonicCheck) return tectonicCheck;
  tectonicCheck = new Promise<{ available: boolean; version: string | null }>((resolve) => {
...
function makeXetexSafe(tex: string): string {
  const shim =
  ...
  if (!match) return tex;
  // A successful non-global .match always sets `index`; assert to satisfy strict.
  const insertAt = match.index! + match[0].length;
  return tex.slice(0, insertAt) + shim + tex.slice(insertAt);
}

export async function compileTexToPdf(tex: string): Promise<Buffer> {
  const status = await checkTectonicAvailability();
  if (!status.available) {
    const error: NodeJS.ErrnoException = new Error("Tectonic is not installed. Install it with `brew install tectonic` to enable in-app PDF rendering.");
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        const error: NodeJS.ErrnoException = new Error("Tectonic timed out after 60s — the LaTeX may contain a non-terminating macro.");
