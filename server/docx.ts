...
export function base64ToBuffer(value: unknown, label = "DOCX"): Buffer {
...
async function withUnpackedDocx<T>(
  docxBase64: unknown,
  callback: (ctx: { workspace: string; unpackedPath: string }) => Promise<T>
): Promise<T> {
...
export async function extractDocxResume(docxBase64: unknown): Promise<{ text: string; paragraphs: number }> {
