// Shared LaTeX → plain-text extractor. This is the single implementation behind
// BOTH the server LaTeX pipeline (server/latex/parseResumeText.ts re-exports it)
// and the client structured editor (src/lib/resumeData.ts imports it) — it used to
// be duplicated as a hand-port between the two, which had to be kept byte-for-byte
// in sync by hand. It's now a plain .ts module: Node runs it directly under its
// native TypeScript type stripping and the bundler/typechecker resolve it the same
// way, exactly like src/lib/jobIdentity.ts / src/resume/sections.ts.
//
// It extracts a plain-text resume from Jake's-style .tex sources so the AI and the
...
// project headings and skills rows. Output is the clean
// "Title | Subtitle | Meta | Location" + "- bullet" format both parsers understand.

// One ordered plain-text event emitted while walking a section body: a heading
// row (`sub`) or a bullet/paragraph row (`item`). Sorted by source `index` then
// insertion `order` before rendering.
type ResumeEvent = { index: number; order: number; kind: "sub" | "item"; text: string };

/**
 * Strip LaTeX line comments (an unescaped `%` to end of line), honoring `\%`.
 */
...
/**
 * Read one brace-balanced group starting at `start` (which must be a `{`).
 */
...
/**
 * Read up to `maxArgs` brace-group arguments of `\command` starting at `index`.
 */
function readCommandArgs(
  ...
  index: number,
  maxArgs = 8
): { index: number; end: number; args: string[] } | null {
...
/**
 * Find every `\command{...}` call in `source`.
 */
function findCommandCalls(
  source: string,
  command: string,
  maxArgs = 8
): { index: number; end: number; args: string[] }[] {
  const calls: { index: number; end: number; args: string[] }[] = [];
  const prefix = `\\${command}`;
  let cursor = 0;
...
/**
 * Replace each `\command{arg}` with its `argIndex`-th argument.
 */
...
/**
 * Replace each `\command{arg}` with its `argIndex`-th arg wrapped in `<tag>…</tag>`.
 */
function replaceCommandWithFormattedArg(
  source: string,
  command: string,
  tag: string,
  argIndex = 0
...
function unwrapLatexInlineCommands(value: string): string {
  ...
  for (let i = 0; i < 6; i += 1) {
    const next = ([
      ...
      ["small", 0],
      ["footnotesize", 0]
    ] as [string, number][]).reduce((current, [command, argIndex]) => replaceCommandWithArg(current, command, argIndex), text);
    if (next === text) break;
    text = next;
  }
  return text;
...
function preserveLatexInlineCommands(value: string): string {
  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    next = ([
      ...
      ["small", 0],
      ["footnotesize", 0]
    ] as [string, number][]).reduce((current, [command, argIndex]) => replaceCommandWithArg(current, command, argIndex), next);
    if (next === text) break;
    text = next;
  }
  return text;
...
function pushItemRows(events: ResumeEvent[], index: number, orderRef: { value: number }, raw: string): void {
...
/**
 * Extract a plain-text resume from a Jake's-style .tex source.
 */
